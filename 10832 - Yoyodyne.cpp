...
struct point3D{
	...
	double x, y, z;
	point3D( double X=0, double Y=0, double Z=1 ): x(X), y(Y), z(Z) {}
	point3D( const point3D& a, const point3D& b ): x(b.x-a.x), y(b.y-a.y), z(b.z-a.z) {}
	double dot( const point3D& p )const{
		return x*p.x + y*p.y + z*p.z;
	}
	double dist2( const point3D& p )const{
		point3D v(*this,p);
		return v.dot(v);
	}
	double cross( const point3D& p )const{
		return geom::cross( Point(x,y), Point(z,p.x), Point(p.y,p.z) );
	}
} p[25], v;
istream& operator>>( istream& in, point3D& x ){ return in >> x.x >> x.y >> x.z; }
int f, n, i, tc, j;
double b, r, len, d, k;
int main(){
	cout.precision(2);
	...
	while( cin >> f >> b >> r >> n, f||b||r||n ){
		...
		for( i=0; i<n; ++i ){
			for( j=i+2; j<=n; ++j ){
				if( dist2(p[i],p[j]) < dist2(p[i],p[i+1]) ) swap( p[j], p[i+1] );
			}
		}
		len = 0.0;
		for( i=0; i<n; ++i ){
		...
		cout << "Mission " << ++tc << ": ";
		if(i==n){
			cout << "SUCCESS!! Time: " << len/r << "  Traveled: " << len << "  Fuel Left: " << f-b*len/r << endl;
		}
		else{
			cout << "FAILURE!! Traveled: " << len << "  From Home: " << d << endl;
		}
	}
}
