...
int main(){
	...
	while(getline(cin,buf)){
		v.clear();
		op = 0;
		for(int i=0,j=i;i<buf.size();i=j+1){
			while(++j<buf.size()){
				if(!isdigit(buf[j]))break;
			}
			switch(op){
				case 0:
					v.push_back( BigInt( buf.substr(i,j-i) ) );
					break;
				case 1: //negative
					v.push_back( -BigInt( buf.substr(i,j-i) ) );
					break;
				case 2:
					v.back() *= buf.substr(i,j-i);
					break;
			}
			if(op==3){
				if(v.back()<TWO) continue; //ignore 0,1
				else{
					istringstream Sin( buf.substr(i,j-i) );
					Sin >> x;
					v.back() = v.back()^x;
				}
			}
		...
		ans.assign('0');
		for(unsigned i=0;i<v.size();i++) ans+=v[i];
		cout << ans << endl;
	}
}
