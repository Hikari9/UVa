...
namespace geom{
	...
	TYPE2 class triangle: public polygon<Type,pType>{
		...
		public:
			...
			pType bary( const Type& ba, const Type& bb, const Type& bc ){ return ( A*ba + B*bb + C*bc )/(ba+bb+bc);; }
...
int main(){
	...
	while(t--){
		...
		if( len >= threshold ) cout << "YES" << endl;
		else cout << "NO" << endl;
	}
}
