...
namespace geom2D{
	...
	TYPE3 class polygon{
		...
		public:
			...
			G_INLINE void GrahamScanCollinear( WiseType Wise = counterclockwise ){
				if( empty() ) return;
				push(vertex(0));
				int m = 2;
				if(Wise==clockwise){
			...
			G_INLINE void GrahamScanCollinearUnique( WiseType Wise = counterclockwise ){
				if( empty() ) return;
				push(vertex(0));
				int m = 2;
				if(Wise==clockwise){
					for(int i=2;i<size();++i){
						while( m>=2 && ( !cw( vertex(m-2), vertex(m-1), vertex(i) ) || (vertex(m-2)==vertex(m-1) && vertex(m-1)==vertex(i)) ) ) --m;
						operator[]( m ) = vertex(i);
						++m;
					}
				}
					...
					for(int i=2;i<size();++i){
						while( m>=2 && ( !ccw( vertex(m-2), vertex(m-1), vertex(i) ) || (vertex(m-2)==vertex(m-1) && vertex(m-1)==vertex(i)) ) ) --m;
						operator[]( m ) = vertex(i);
						++m;
					}
				}
			...
			G_INLINE polygon& ConvexHullCollinear( WiseType Wise = counterclockwise ){
				sort( Wise );
				GrahamScanCollinear( Wise );
				return *this;
			}
			
			G_INLINE polygon& ConvexHullCollinearUnique( WiseType Wise = counterclockwise ){
				sort( Wise );
			...
			G_INLINE polygon hullCollinear( WiseType Wise = counterclockwise ) const{
				return polygon( *this ).ConvexHullCollinear( Wise );
			}
			
			G_INLINE polygon hullCollinearUnique( WiseType Wise = counterclockwise ) const{
...
int c, r, o, i, tc;
polygon<int> cops, robbers;
point<int> citizen;
bool isSafe, isRobbed;
int main(){
	while( cin >> c >> r >> o, c||r||o ){
		...
		cin >> cops >> robbers;
		// cout << "COPS: " << cops << endl;
		// cout << "ROBS: " << robbers << endl;
		cops.ConvexHullCollinear();
		robbers.ConvexHullCollinear();
		// uniqueify
		if( cops.size()<3 ) cops.clear();
		if( robbers.size()<3 ) robbers.clear();
		// cout << "COPS: " << cops << endl;
		// cout << "ROBS: " << robbers << endl;
		...
		while( o-- ){
			...
			isSafe = ( citizen.isInPolygon(cops) || (citizen.isOnPolygon(cops)) );
			isRobbed = ( citizen.isInPolygon(robbers) || (citizen.isOnPolygon(robbers)) );
			if( !isSafe && !isRobbed ) cout << "neither.";
			else if( isSafe ) cout << "safe.";
			else cout << "robbed.";
			cout << endl;
		}
		cout << endl;
