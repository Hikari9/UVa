...
namespace geom{
	...
	TYPE1 point<Type> make_point( const Type& a, const Type& b ){ return point<Type>(a,b); }
	
	TYPE2 segment<typename Type::value_type,Type> make_segment( const Type& a, const pType& b ){ return segment<typename Type::value_type,Type>(a,b); }
	...
	TYPEpp dist( const pType& a, const pType2& b ){ return sqrt( dist2(a,b) ); }
	TYPEpp cross( const pType& a, const pType2& b ){ return a.cross(b); }
	TYPEppp cross( const pType& a, const pType2& b, const pType3& c ){ return a.cross(b,c); }
	...
	TYPE3o oType rotate( const oType& a, const oType2& b, const oType3& t ){ return a+(b-a).rotate(t); }
	...
	namespace versatile{
		...
		TYPE2 bool inRange( const point<pType>& p, const segment<Type,point<pType> >& s ){
			return std::greater_equal<pType>()( p.x, std::min( s.A.x, s.B.x ) )
				&& std::less_equal<pType>()( p.x, std::max( s.A.x, s.B.x ) )
				&& std::greater_equal<pType>()( p.y, std::min( s.A.y, s.B.y ) )
				&& std::less_equal<pType>()( p.y, std::max( s.A.y, s.B.y ) );
		}
		
		TYPE2 pType lattice( const segment<Type,point<pType> >& s ){
			return geom::math::gcd( geom::math::abs( dx(s.A,s.B) ), geom::math::abs( dy(s.A,s.B) ) );
		}
		
		TYPE2 point<pType> bary( const triangle<Type,point<pType> >& t, const Type& ba, const Type& bb, const Type& bc ){
			Type sum = ba+bb+bc;
			return point<pType>( ( t.A.x*ba + t.B.x*bb + t.C.x*bc )/sum, ( t.A.y*ba + t.B.y*bb + t.C.y*bc )/sum );
		}
...
#undef MAKE_STREAM2
#undef MAKE_STREAM3
#define MAKE_STREAM1(T,OP,T2) \
	TYPEo std::T& operator OP( std::T&, geom::T2<oType>& )
#define MAKE_STREAM2(T,OP,T2) \
	TYPE2o std::T& operator OP( std::T&, geom::T2<oType,oType2>& )
#define MAKE_STREAM3(T,OP,T2) \
	TYPE3o std::T& operator OP( std::T&, geom::T2<oType,oType2,oType3>& )

MAKE_STREAM1(istream,>>,point);
MAKE_STREAM1(ostream,<<,point);

MAKE_STREAM2(istream,>>,line);
// MAKE_STREAM2(istream,>>,segment);
MAKE_STREAM2(istream,>>,circle);
MAKE_STREAM2(ostream,<<,line);
// MAKE_STREAM2(ostream,<<,segment);
MAKE_STREAM2(ostream,<<,circle);

MAKE_STREAM3(istream,>>,polygon);
MAKE_STREAM3(ostream,<<,polygon);
...
namespace geom{
	...
	TYPE1 class point{
		public:
			...
			TYPE2o point( const point<oType>& p1, const point<oType>& p2 ): x(dx(p1,p2)), y(dy(p1,p2)) {}
			...
			TYPEo Type dist2( const point<oType>& p )const{
				Type DX = dx(*this,p), DY = dy(*this,p);
				return ( DX*DX + DY*DY );
			}
			TYPEo Type dot( const point<oType>& p )const{ return x*p.x+y*p.y; }
			TYPEo Type cross( const point<oType>& p )const{ return x*p.y-p.x*y; }
			TYPE2o Type cross( const point<oType>& b, const point<oType2>& c )const{ return point( dx(*this,b), dy(*this,b) ).cross(point( dx(*this,c), dy(*this,c) ) ); }
			...
			#define MAKE_FUNCTION(OP,OP2) \
				...
				TYPEo point& operator OP2 ( const point<oType>& p )const{ x OP2 p.x, y OP2 p.y; return *this; }
			#undef SCALAR_FUNCTION
			#define SCALAR_FUNCTION(OP,OP2) \
				point operator OP( const Type& p )const{ return point( x OP p, y OP p ); } \
				point& operator OP2 ( const Type& p )const{ x OP2 p, y OP2 p; return *this; }
				MAKE_FUNCTION(+,+=)
				MAKE_FUNCTION(-,-=)
				SCALAR_FUNCTION(+,+=)
				SCALAR_FUNCTION(-,-=)
	...
	TYPE2 class line{
		...
		public:
			...
			Type angle()const{ return atan2( dy(A,B), dx(A,B) ); }
			...
			Type slope()const{ if( isVertical() ) return std::numeric_limits<Type>::quiet_NaN(); return dy(A,B)/dx(A,B); }
			
			Type x_intercept()const{ return isVertical() ? A.x : -y_intercept()/slope(); }
			Type y_intercept()const{ return A.y - slope()*A.x; }
			Type x_value( const Type& x )const{ return isVertical() ? A.x : slope()*x+y_intercept(); }
			Type y_value( const Type& y )const{ isHorizontal() ? A.y : (y-y_intercept())/slope(); }
			
			TYPE2o bool equals( const line<oType,oType2>& l )const{ return std::equal_to<Type>()( slope(), (Type)l.slope() ); }
			
			std::string toString()const{ return A.toString() + " " + B.toString(); }
			
			TYPE2o bool parallel( const line<oType,oType2>& l )const{ return std::equal_to<Type>()( cross(B-A,l.B-l.A), 0 ); }
			TYPE2o bool collinear( const line<oType,oType2>& l )const{ return parallel(l) && std::equal_to<Type>()( cross(A-l.A,B-l.B), 0 ); }
			...
			TYPE2o pType* intersection( const segment<oType,oType2>& s )const{
				...
				if( i!=NULL && s.inRange(*i) ) return i;
				return NULL;
			}
	...
	TYPE2 class segment: public line<Type,pType>{
		...
		public:
			...
			bool inRange( const pType& p )const{ return geom::versatile::inRange(p,*this); }
			typename pType::value_type lattice()const{ return geom::versatile::lattice(*this); }
			
			bool contains( const pType& p )const{
				return inRange(p) && std::equal_to<Type>()( (Type)cross(this->A,p,this->B), (Type)0 );
			}
			segment() {}
			...
			TYPE2o pType* intersection( const line<oType,oType2>& l )const{
				...
				if( i!=NULL && inRange(*i) ) return i;
				return NULL;
			}
			
			TYPE2o pType* intersection( const segment<oType,oType2>& s )const{
				pType* i = line<Type,pType>(*this).intersection( line<oType,oType2>(s) );
				if( i!=NULL && inRange(*i) && s.inRange(*i) ) return i;
				return NULL;
			}
		...
		public:
			...
			Type boundary()const{
				...
				for( int i=0, j=size()-1; i<size(); j=i++ ){
					bound += make_segment( vertex(i), vertex(j) ).lattice();
				}
				return bound;
			}
	...
	TYPE2 class triangle: public polygon<Type,pType>{
		...
		public:
			...
			pType bary( const Type& ba, const Type& bb, const Type& bc ){ return versatile::bary( *this, ba, bb, bc ); }
...
#undef MAKE_STREAM3
#define MAKE_STREAM1(T,OP,T2) \
	TYPEo std::T& operator OP( std::T& s, geom::T2<oType>& o )
#define MAKE_STREAM2(T,OP,T2) \
	TYPE2o std::T& operator OP( std::T& s, geom::T2<oType,oType2>& o )
#define MAKE_STREAM3(T,OP,T2) \
	TYPE3o std::T& operator OP( std::T& s, geom::T2<oType,oType2,oType3>& o )

MAKE_STREAM1(istream,>>,point){ return s >> o.x >> o.y; }
MAKE_STREAM1(ostream,<<,point){ return s << o.x << ' ' << o.y; }

MAKE_STREAM2(istream,>>,line){ return s >> o.A >> o.B; }
MAKE_STREAM2(istream,>>,circle){ return s >> o.center >> o.r; }
MAKE_STREAM2(ostream,<<,line){ return s << o.A << ' ' << o.B; }
MAKE_STREAM2(ostream,<<,circle){ return s << o.center << ' ' << o.r; }

MAKE_STREAM3(istream,>>,polygon){ for( int i=0; i<o.size(); ++i ) s >> o[i]; return s; }
MAKE_STREAM3(ostream,<<,polygon){ for( int i=0; i<o.size(); ++i ){ if(i) s << ' '; s << o[i]; } return s; } 
...
int main(){
	while( cin >> n, n ){
		...
		i = p.intersection(p2);
		i.ConvexHull();
		printf( "%8.2lf", p.area()+p2.area()-2.0*i.area() );
	}
	putchar('\n');
}
