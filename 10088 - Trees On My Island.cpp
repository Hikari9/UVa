...
int n;
geom2D::polygon<lag> p;
lag boundary( geom2D::polygon<lag>& P ){
	lag sum = 0;
	for(int i=0, j=n-1;i<n;j=i++){
		sum += gcd(llabs(dx(P[i],P[j])), llabs(dy(P[i],P[j])));
	}
	return sum;
}
int main(){
	while(cin >> n,n){
		...
		// p.GrahamScan();
		// cout << p.signedArea() << endl;
		cout << (llabs(p.signedArea())-boundary(p)+2LL)/2LL << endl;
	}
}
