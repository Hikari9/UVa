...
enum degreeType{ deg=360, rad, grad=36000 };
static degreeType degree = rad;

TYPE1 Type RAD( const Type& _ ){ return _ * PI/180.; }
TYPE1 Type DEG( const Type& _ ){ return _*180./PI; }
...
namespace geom2D{
	...
	TYPE1 class point{
		...
		public:
			...
			Type cross_product( const point& _ )const{ return dy(_,*this) * (x+_.x); }
	...
	TYPE1 class line{
		...
		public:
			...
			line& assign( const ray<Type>& _ ){ Normalize(_.vertex, point<Type>(_.vertex.x+cos(_.theta),_.vertex.y+sin(_.theta))); }
			...
			TYPEo bool operator<( const oType& _ )const{ return m==_.m ? b < _.b : m < _.m; }
			TYPEo bool operator<=( const oType& _ )const{ return operator<(_)||operator==(_); }
			TYPEo bool operator>( const oType& _ )const{ return m==_.m ? b > _.b : m > _.m; }
	...
	TYPE1 class segment{
		public:
			...
			struct inRange{
				bool x( const Type& X )const{ return X>=min(A.x,B.x) && X<=max(A.x,B.x); }
				bool y( const Type& Y )const{ return Y>=min(A.y,B.y) && Y<=max(A.y,B.y); }
				bool xy( const Type& X, const Type& Y )const{ return x(X)&&y(Y); }
				bool xy( const point<Type>& _ )const{ return x(_.x) && y(_.y); }
			};
			...
			Type y_value( const Type& x )const{
				Type y = line<Type>(A,B).y_value( x );
				if(inRange().y(y)) return y;
				return std::numeric_limits<Type>::quiet_NaN();
			}
			
			Type x_value( const Type& y )const{
				Type x = line<Type>(A,B).x_value( y );
				if(inRange().x(x)) return x;
				return std::numeric_limits<Type>::quiet_NaN();
			}
			...
			bool operator==( const segment& _ )const{ return (A==_.A && B==_.B); }
			TYPEo bool operator==( const oType& _ )const{ return false; }
			TYPEo bool operator!=( const oType& _ )const{ return !operator==(_); }
			TYPEo bool operator<( const oType& _ )const{ return length() < _.length(); }
			TYPEo bool operator<=( const oType& _ )const{ return operator<(_)||operator==(_); }
			TYPEo bool operator>( const oType& _ )const{ return length() > _.length(); }
			TYPEo bool operator>=( const oType& _ )const{ return operator>(_)||operator==(_); }
	...
	TYPE3 class polygon{
		...
		private:
			...
			struct arrange{
				pType val;
				comp cmp;
				arrange( const pType& v ): val(v) {}
				bool operator()( const pType& lhs, const pType& rhs){
					return cmp( angle( val, lhs ), angle ( val, rhs ) );
				}
			};
		...
		public:
			...
			void GrahamScan( WiseType Wise = clockwise ){
				swap(operator[](0u),*min_element( v.begin(), v.end() ));
				if(Wise==clockwise) sort(v.begin()+1,v.end(),arrange< std::greater<Type> >(vertex(0)));
				else sort(v.begin()+1,v.end(),arrange< std::less<Type> >(vertex(0)));
			}
	...
	TYPE2 class triangle: public polygon<Type,pType>{
		public:
			...
			virtual void GrahamScan( WiseType Wise = clockwise ){
				...
				swap(*mn,A);
				if( Wise==clockwise ){
					if( angle(A,B) < angle(A,C) ) swap(B,C);
				}
				else{
					if( angle(A,B) > angle(A,C) ) swap(B,C);
				}
	...
	TYPE2 class circle{
		public:
			...
			Type arcLength( const Type& _ )const{ return circumference()*_/(degree==rad?PI:degree); }
			Type chordLength( const Type& _ )const{ return sqrt( (r*r*2)*(1-cos(_))); }
			Type sectorArea( const Type& _ )const{ return area()*_/(degree==rad?PI:degree); }
...
TYPE2o std::istream& operator>>( std::istream& in, geom2D::triangle<oType,oType2>& tr ){ in >> tr.A >> tr.B >> tr.C; tr.update(); return in; }
...
TYPE2o std::ostream& operator<<( std::ostream& out, const geom2D::circle<oType,oType2>& cc ){ return out << cc.center << std::endl << cc.r; }
TYPE2o std::ostream& operator<<( std::ostream& out, const geom2D::triangle<oType,oType2>& tr ){ return out << tr.A << ' ' << tr.B << ' ' << tr.C; }
TYPE3o std::ostream& operator<<( std::ostream& out, const geom2D::polygon<oType,oType2,oType3>& pl ){ for(unsigned i=0;i<pl.size();++i) out << pl.vertex(i) << std::endl; return out; }
...
int main(){
	...
	while(cin >> c,c!='*'){
		switch(c){
			case 'r':
				v1.push_back( make_pair( Polygon(4), r ) );
				cin >> A >> B;
				v1.back().first[0] = A;
				v1.back().first[1] = Point(A.x,B.y);
				v1.back().first[2] = B;
				v1.back().first[3] = Point(B.x,A.y);
				break;
			case 't':
				v1.push_back( make_pair( Polygon(3), r ) );
				cin >> v1.back().first;
				break;
			case 'c':
				 v2.push_back( make_pair( Circle(), r ) );
				 cin >> v2.back().first;
				 break;
		}
		++r;
	...
	r=1;
	while( cin >> P, P.x!=9999.9 || P.y!=9999.9 ){
		for(int i=0;i<v1.size();++i) if( P.isInPolygon(v1[i].first)) pq.push_back(v1[i].second);
		for(int i=0;i<v2.size();++i) if( P.isInCircle(v2[i].first)) pq.push_back(v2[i].second);
		if(pq.empty()){
			cout << "Point " << r << " is not contained in any figure" << endl;
		}
		else{
			sort(pq.begin(),pq.end());
			for(int i=0;i<pq.size();++i) cout << "Point " << r << " is contained in figure " << pq[i] << endl;
			pq.clear();
		}
		++r;
	}
