...
string s;
int st, i, mx;
bool isPalindrome( const BigInt& x ){
	for( int j=0, k=x.size()/2; j<k; ++j ){
		if( x[j]!=x.digit(j) ) return false;
	}
	return true;
}
BigInt addPalindrome( const BigInt& x ){
	BigInt ans(x);
	reverse( ans.access().begin(), ans.access().end() );
	return ans += x;
}
int countSteps( const BigInt& x ){
	if( isPalindrome(x) ) return 0;
	return countSteps( addPalindrome( x ) ) + 1;
}
int main(){
	while( cin >> s ){
		mx = 1;
