...
namespace geom2D{
	...
	TYPE1 class segment{
		...
		public:
			...
			G_INLINE bool contains( const point<Type>& _ )const{ return cross(A,_,B)==0; }
...
int main(){
	...
	while( t-- ){
		...
		p.sort(counterclockwise);
		// swap( p[0], *min_element( p.access().begin(), p.access().end(), xCMP ) );
		// sort( p.access().begin()+1, p.access().end(), xTHETACMP );
		// p.GrahamScan();
		n = p.size();
		cout << n << endl;
