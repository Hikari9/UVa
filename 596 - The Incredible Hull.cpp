...
namespace geom2D{
	...
	TYPE1 class point{
		...
		public:
			...
			G_INLINE bool operator<( const point& _ )const{ return y==_.y ? x<_.x : y<_.y; }
			G_INLINE bool operator<=( const point& _ )const{ return y==_.y ? x<=_.x : y<_.y; }
			G_INLINE bool operator>( const point& _ )const{ return y==_.y ? x>_.x : y>_.y; }
			G_INLINE bool operator>=( const point& _ )const{ return y==_.y ? x>=_.x : y>_.y; }
	...
	TYPE3 class polygon{
		...
		public:
			struct arrange_cw{
				...
				bool operator()( const pType& a, const pType& b )const{
					Type cp = cross( *val, a, b );
					if(!cp) return dist2(*val,a)>dist2(*val,b);
					return 0>cp;
				}
			};
			struct arrange_ccw{
			...
			G_INLINE bool ccw( const pType& a, const pType& b, const pType& c ){ return cross(a,b,c)>=0; }
			G_INLINE bool cccw( const pType& a, const pType& b, const pType& c ){ return cross(a,b,c)<=0; }
		...
		public:
			...
			G_INLINE void sort( WiseType Wise = clockwise ){
			...
			G_INLINE void GrahamScan( WiseType Wise = clockwise){
				push(vertex(0));
				int m = 2;
				if(Wise==clockwise){
			...
			G_INLINE polygon& ConvexHull( WiseType Wise = clockwise ){
				sort( Wise );
				GrahamScan( Wise );
				return *this;
			}
			
			G_INLINE polygon hull( WiseType Wise = clockwise )const{
	...
	TYPE2 class triangle: public polygon<Type,pType>{
		...
		public:
			...
			G_INLINE virtual void GrahamScan( WiseType Wise = clockwise ){
				pType* mn;
				if(A<B){
					if(A<C) mn = &A;
					else mn = &C;
...
typedef int lag;
bool xCmp( const point<lag>& a, const point<lag>& b ){
	return a.x==b.x ? a.y < b.y : a.x > b.x;
}
polygon<lag> P, h, hull;
point<lag> pt;
string cmd;
char c;
int n, i, mni;
int main(){
	...
	while( c!='E' ){
		...
		h = P.hull();
		cout << cmd << " convex hull:" << endl;

		hull.clear();
		int j, k;
		for( i=0; i<P.size(); ++i ){
			if( find( h.access().begin(), h.access().end(), P[i] )!=h.access().end() ) continue;
			for( j=0, k=h.size()-1; j<h.size(); k=j++ ){
				if( cross(h[k],P[i],h[j])==0 ){
					hull += P[i];
					break;
				}
			}
		...
		h += hull;
		// find largest x, then largest y, sort h
		mni = 0;
		for( i=1; i<h.size(); ++i ){
			if( xCmp( h[i], h[mni] ) ) mni = i;
		}
		if( mni!=0 ) swap( h[mni], h[0] );
		sort( h.access().begin()+1, h.access().end(), polygon<lag>::arrange_ccw( h[0] ) );
