...
namespace geom2D{
	...
	TYPE3 class polygon{
		...
		public:
			...
			polygon& ConvexHull( WiseType Wise = clockwise ){
				sort( Wise );
				GrahamScan( Wise );
				return *this;
			}
...
struct node{
	...
	double A;
	node():i(),j(),k(),A() {}
	node( int I, int J, int K, double a ): i(I), j(J), k(K), A(a) {}
};
Point p[15];
char c[15], ans[4];
int n, i, j, k, l;
			...
			for( j=i+1; j<n; ++j ){
				for( k=j+1; k<n; ++k ){
					...
					for( l=0; l<n; ++l ){
						...
						if( p[l].isInPolygon( t ) || p[l].isOnPolygon( t ) ) break;
					}
					if( l==n ){
						double a = abs(t.signedArea());
						if( mx.A < a ) mx = node(i,j,k,a);
					}
				}
			}
		}
