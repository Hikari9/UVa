...
bool ok(){
	for( int i=0; i<3; ++i ){
		if( ABC[i].x!=0.0 || ABC[i].y!=0.0 ) return true;
		if( DEF[i].x!=0.0 || DEF[i].y!=0.0 ) return true;
	}
	return false;
}
int main(){
	...
	while( cin >> ABC >> DEF ){
		...
		// cout << ABC.toString() << " " << DEF.toString() << endl;
		// cout << ABC.area() << " " << area << endl;
		lo = ABC.A;
		hi = ABC.C;
		while( dist(lo,hi) > 1e-7 ){
			mid = (lo+hi)/2.0;
			if( abs( cross( mid, ABC.A, ABC.B ) ) < area ){
				lo = mid;
			}
			else{
				hi = mid;
